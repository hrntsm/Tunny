...
namespace BayesOpt
{
    public class OptunaGHComponent : GH_Component
    {
        private int _i = 0;
        ...
        public OptunaGHComponent()
          : base("Optuna GH Component", "Nickname",
                ...
                if (obj is Grasshopper.Kernel.Special.GH_Group group && group.NickName == "Variables")
                {
                    foreach (IGH_DocumentObject gObj in group.Objects())
                    {
                        if (gObj is Grasshopper.Kernel.Special.GH_NumberSlider s && _state != SolverState.Completed)
                        {
                            ...
                            // s.Slider.RaiseEvents = false;
                            s.SetSliderValue(val);
                            // s.ExpireSolution(false);
                            // s.Slider.RaiseEvents = true;
                        }
                    }
                }
            }
        ...
        public override Guid ComponentGuid => new Guid("833bf9a8-ab07-47fb-b628-694fd52375dc");
    }
}